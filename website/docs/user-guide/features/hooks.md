...
- Several hooks' return values affect behavior: [`pre_tool_call`](#pre_tool_call) can **block** the tool, [`pre_llm_call`](#pre_llm_call) can **inject context** into the LLM call, and security-sensitive hooks (e.g. [`pre_gateway_authorize_message`](#pre_gateway_authorize_message)) document their fail-open/fail-closed rules explicitly below. All other hooks are fire-and-forget observers.
